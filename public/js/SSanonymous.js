  ...
  firebase.database().ref("Courses/" + routeParams.Course_ID).once('value').then(snap3 => {
    firebase.database().ref("Anonymous/" + routeParams.Course_ID + "/" + routeParams.AnonPost_ID).once('value').then(snap => {
      ...
      firebase.database().ref("Users/" + snap.val().User_ID).once('value').then(snap2 => {
        ...
        if (firebase.auth().currentUser.uid == snap.val().User_ID || firebase.auth().currentUser.uid == snap3.val().Admin) {
          $("#title_row").append(" <a href='javascript:void(0)' class='btn btn-outline-danger' onClick='deleteAnonPost(\"" + routeParams.Course_ID + "\", \"" + routeParams.AnonPost_ID + "\")'>Delete Post</a></div></p>");
        }
      });
